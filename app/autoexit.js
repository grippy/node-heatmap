   ...
   (function()
    {
     ...
     function(err,stats)
     {
      ...
      else
      {
       ...
       {    
            // hack to prevent the socket.io client script from triggering a restart in dev mode
            // sys.puts(restart_server)
            if (file_on_callback.indexOf('socket.io')==-1 && file_on_callback.indexOf('redis-node')==-1){
